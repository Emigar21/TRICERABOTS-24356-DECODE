...
public class Blue_Alliance_TeleOp extends OpMode {
    ...
    public void loop() {
        ...
        if (RT2 != 0) {
            ...
            subsystems.intake.moveIntake(Shooter.getActualVel() >= (Shooter.controlPoints.get(range) - 600) ? 1 : 0);
            subsystems.indexer.moveIndexer(Shooter.getActualVel() >= (Shooter.controlPoints.get(range) - 600) ? 1 : 0);
            subsystems.feeder.moveFeeder(Shooter.getActualVel() >= (Shooter.controlPoints.get(range) - 600) ? 1 : 0);
        } else {
            autoMode = false;
